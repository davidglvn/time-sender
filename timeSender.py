...
def create_new_ticket(time_entry, agent_id):
...
def scan_toogl_result(time_entries, agent_id):
    if not agent_id:
        return False
    ...
    for time_entry in time_entries:
        ...
        try:
            if 'new-ticket' not in time_entry['tags']:
                ...
                ticket_id = create_new_ticket(time_entry, agent_id)
        except KeyError:
...
if __name__ == '__main__':
    ...
    while True:
        if start_time <= datetime.datetime.now() - datetime.timedelta(hours=6):
            ...
            scan_toogl_result(get_data_from_toogl(), get_freshdesk_agent_id())
